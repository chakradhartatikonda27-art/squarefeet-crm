...
export default function SalesHome() {
  ...
  const warmLeads = [
    { name: 'Venkat Rao', area: 'Rushikonda', budget: '₹1.2Cr', stage: 'Site Visit', note: 'Very interested, wants 4BHK villa', sbg: '#FFF3E0', sfg: '#E65100' },
    { name: 'Naresh Reddy', area: 'Bheemili', budget: '₹2.1Cr', stage: 'Negotiation', note: 'Discussing price, wants 5% discount', sbg: '#F3E5F5', sfg: '#6A1B9A' },
    { name: 'Durga Prasad', area: 'Rushikonda', budget: '₹1.8Cr', stage: 'Site Visit', note: 'Second visit scheduled today', sbg: '#FFF3E0', sfg: '#E65100' },
    { name: 'Ramesh Babu', area: 'MVP Colony', budget: '₹95L', stage: 'Interested', note: 'Needs loan assistance info', sbg: '#E8F5E8', sfg: '#2D8529' },
  ];

  const todayVisits = [
    { name: 'Durga Prasad', time: '11:00 AM', property: 'Rushikonda Heights — Villa B4', status: 'Upcoming' },
    { name: 'Venkat Rao', time: '3:00 PM', property: 'Rushikonda Heights — Villa C2', status: 'Confirmed' },
  ];
    ...
    <div style={{minHeight: '100dvh', background: '#F0F2F8', display: 'flex', flexDirection: 'column'}}>
      ...
      <div style={{flex: 1, padding: '14px', overflowY: 'auto', paddingBottom: '80px'}}>
          ...
          {todayVisits.map((visit, i) => (
            <div key={i} style={{padding: '12px 14px', borderBottom: '1px solid #DDE2EF',
              ...
              <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                ...
                <span style={{fontSize: '12px', fontWeight: '700', color: '#F57C00'}}>
                  {visit.time}
                </span>
              </div>
        ...
        <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px'}}>
          {[
            { icon: '🔥', label: 'Warm Leads', sub: '8 assigned to me', color: '#1B2F6E', bg: '#E8EBF5', border: '#1B2F6E' },
            { icon: '✅', label: 'Booking', sub: 'Confirm a booking', color: '#3AAA35', bg: '#E8F5E8', border: '#3AAA35' },
            { icon: '📁', label: 'Documents', sub: 'Upload files', color: '#F57C00', bg: '#FFF3E0', border: '#F57C00' },
            { icon: '📍', label: 'Attendance', sub: 'Check in/out', color: '#2E9FD4', bg: '#E3F4FB', border: '#2E9FD4', path: '/sales/attendance' },
          ].map((btn, i) => (
            <button key={i} onClick={() => btn.path && router.push(btn.path)}
            ...
            <div key={i} style={{padding: '12px 14px', borderBottom: '1px solid #DDE2EF',
              ...
              <div style={{display: 'flex', gap: '6px'}}>
                ...
                <button style={{flex: 1, padding: '7px', borderRadius: '8px',
                                ...
                                color: '#F57C00', fontSize: '11px',
                                fontWeight: '700', cursor: 'pointer'}}>
                  📍 Site Visit
                </button>
                <button style={{flex: 1, padding: '7px', borderRadius: '8px',
                                border: '1.5px solid #3AAA35', background: '#E8F5E8',
                                color: '#2D8529', fontSize: '11px',
                                fontWeight: '700', cursor: 'pointer'}}>
                  ✅ Book
                </button>
              </div>
